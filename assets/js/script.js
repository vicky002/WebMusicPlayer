...
var allTracks = [],			playlist = [], 			temporarySearchPlaylist = [],		i = 0, 					shuffle = false,		repeat = 0,				lastPlayed = [],		timer = 0;			
...
function getTags(file,done){
	...
	ID3.loadTags(file.name, function() {
		...
		if(tags.picture && tags.picture.data && tags.picture.data.length) {
			...
			result.picture = 'assets/img/streaming.jpg';
			done(result);
		}
...
wavesurfer.on('ready', function () {
	...
	if(playlist[i]){
		...
		if(playlist[i].picture == 'assets/img/streaming.jpg'){
...
$('#playlist').on('click', function (e) {
	...
	if(index!=undefined){
			...
			if(!playlist.length){
					...
					clearInterval(timer);
					$('#cover-art-big').css("background", "");
					$('#cover-art-small').attr('src', 'assets/img/streaming.jpg');
